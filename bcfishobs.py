...
@click.option("--db_url", help="Target database Default: $FWA_DB", envvar="FWA_DB")
def download(email, db_url):
    ...
    url = (
        "https://hillcrestgeo.ca/outgoing/whse_fish/whse_fish.wdic_waterbodies.csv.zip"
    )
    ...
    gdal_pg = "PG:host='{h}' port='{p}' dbname='{db}' user='{usr}' password='{pwd}'".format(
        h=u.hostname, p=u.port, db=u.path[1:], usr=u.username, pwd=u.password
    )
    gdal.VectorTranslate(
        ...
        layerCreationOptions=["OVERWRITE=YES", "SCHEMA=whse_fish"],
    )
    gdal.VectorTranslate(
        gdal_pg,
        "species_cd.csv",
